...
typedef struct __longConstantReference {
	u2 classId;
	// Index into constant pool of the referencing class:
	u2 constantPoolIndex;
	jlong value;
} longConstantReference;
...
typedef struct __classInstanceInfoDef {
	...
	// The size of an instance in counts of stackables:
	u2 instanceSize;
...
extern const u2 numberOfAllLongConstantReferences;
extern const longConstantReference const allLongConstantReferences[];
...
/**
 * This function looks up a field in a class
 * \param classId Identifies the class
 * \param linkId The id to look up with
 * \return A pointer to the fieldInClass struct. If no match => no return
 */
 ...
 */
u2 cpGetMemberBuildinDependecy(BuildinDependency_e libref);
#endif

/**
 * This method looks up value (int, float or String) from within const pool
 * \param constantPoolIndex Identifies the value to look up
 *
 */
...
/**
 * Macro for type validation
 */
#if 1
#define VALIDATE_TYPE(TYPE, EXPECT_TYPE) \
	if ((TYPE) != (EXPECT_TYPE)) {\
		consout("%s %d: Expected type %s got %s(%d)\n", __FILE__, __LINE__, \
				StackTypeToString(EXPECT_TYPE), StackTypeToString(TYPE), TYPE );\
				exit(1);\
		jvmexit(1);\
	}
#else
